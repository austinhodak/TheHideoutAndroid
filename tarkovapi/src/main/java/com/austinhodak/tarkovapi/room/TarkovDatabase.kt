...
@Database(entities = [Item::class, Ammo::class, Weapon::class, Craft::class, Barter::class, Quest::class, PriceItem::class], version = 19)
...
abstract class TarkovDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        fun getDatabase(
            ...
            //Database prepoluates on first run.

            return INSTANCE ?: synchronized(this) {
                val instance = Room.databaseBuilder(context.applicationContext, TarkovDatabase::class.java, "item_database")
                    .fallbackToDestructiveMigration()
                    .addCallback(DatabaseCallback(context, scope))
                    .createFromAsset("assets/item_database.db")
                    .build()
    ...
    fun updatePricing(api: ApolloClient, scope: CoroutineScope) {
        scope.launch {
            val response = api
                .query(ItemsByTypeQuery(com.austinhodak.tarkovapi.type.ItemType.ANY))
